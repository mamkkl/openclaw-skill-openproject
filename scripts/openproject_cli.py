...
class OpenProjectClient:
    ...
    def add_comment(self, work_package_id: int, comment: str) -> Dict[str, Any]:
        ...
        Uses POST to /work_packages/{id}/activities which is the canonical
        OpenProject API v3 method for adding comments (journal entries).
        """
        work_package = self.get_work_package(work_package_id)

        # Use the addComment link from the work package if available,
        # otherwise fall back to the conventional activities path.
        add_comment_link = nested_get(work_package, ["_links", "addComment"], None)
        if isinstance(add_comment_link, dict):
            href = add_comment_link.get("href", "")
            if href:
                post_path = to_api_path(href)
            else:
                post_path = f"/work_packages/{work_package_id}/activities"
        else:
            post_path = f"/work_packages/{work_package_id}/activities"
        ...
        # Sanity-check: the response should be an Activity, not a new WorkPackage.
        result_type = result.get("_type", "")
        if result_type == "WorkPackage" and result.get("id") != work_package_id:
            raise OpenProjectError(
                f"Unexpected response: server returned a new WorkPackage "
                f"(#{result.get('id')}) instead of an Activity. "
                f"Comment may not have been added to #{work_package_id}."
            )
...
def format_activity(activity: Dict[str, Any], show_changes: bool = False) -> str:
    ...
    """
    author = link_title(activity, "user")
    date = format_date(activity.get("createdAt", ""))
    comment = extract_formattable_text(activity.get("comment"))

    lines = [f"Author: {author}", f"Date: {date}"]

    if comment:
        lines.append(comment)
    elif show_changes:
        details = activity.get("details", [])
        changes = [extract_formattable_text(d) for d in details]
        changes = [c for c in changes if c]
        if changes:
            lines.append("[Field changes: " + "; ".join(changes) + "]")
    return "\n".join(lines)
...
def print_activities(activities: List[Dict[str, Any]], show_changes: bool = False) -> None:
    ...
    for i, activity in enumerate(activities):
        if i > 0:
            print("---")
        print(format_activity(activity, show_changes=show_changes))
...
def command_add_comment(args: argparse.Namespace) -> None:
    ...
    # Always report the original work package ID — the response may be
    # an activity object (from the activities endpoint) rather than the WP itself.
...
def command_list_comments(args: argparse.Namespace) -> None:
    ...
    client = build_client_from_env()
    activities = client.get_activities(args.id)
    raw_data = activities  # keep for --debug-json

    # Unless --all, filter to activities with non-empty comment text
    if not args.all:
        activities = [
            a for a in activities
            if extract_formattable_text(a.get("comment"))
        ]

    # Apply --author filter if provided
    if args.author:
        activities = filter_activities_by_author(activities, args.author)
    ...
    if not activities:
        if args.author:
            ...
            print(f"No comments found for work package #{args.id}.")
    else:
        print_activities(activities, show_changes=args.all)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser_list_comments = subparsers.add_parser(
        "list-comments",
        help="List comments on a work package.",
        description="Fetch and display comments/activities for a work package.",
    )
    parser_list_comments.add_argument("--id", type=int, required=True, help="Work package ID.")
    parser_list_comments.add_argument(
        "--all",
        action="store_true",
        help="Show all activities including field changes (not just comments).",
    )
    parser_list_comments.add_argument(
        "--author",
